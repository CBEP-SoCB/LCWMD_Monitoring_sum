...
better to think about chlorides as a measure of salinity. Physiologically,
...
of some of those models are available in the more complete SoCB GitHub 
...
simpler model, principally bvecause it makes explaining the model more direct.
...
$$ 
log(\text{Chlorides}) = f(\text{Covariates}) +
\beta_{1,i} \text{Site}_i + 
\beta_{2,j} \text{Month}_j + \beta_3 \text{Year} + \beta_{4,i} \text{Site}_i * \text{Year} + u_{t}
$$

$$
u_{t} = a_{1} u_{t−1} + \epsilon_{t} \text{ is the autocorrelated error}
$$
...
On the whole, these models are OK, but not great. They have heavy tailed, skewed 
