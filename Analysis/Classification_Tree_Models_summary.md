Classificatin Tree Models of ‘Diurnal Exceedances’ on Long Creek
================
...
-   [Days that Pass DO Standards](#days-that-pass-do-standards)
    ...
    -   [Side Bar: Frequency of Watershed Low
        Flow](#side-bar-frequency-of-watershed-low-flow)
        -   [Low Flow in the Upper
            Watershed](#low-flow-in-the-upper-watershed)
    -   [DO Model Based on Local Water
        Depth](#do-model-based-on-local-water-depth)
...
We explore classification tree models that look for partitions of the
data that do a good job predicting whether water quality criteria are
met, based on splitting the data into groups based on values of
predictor variables. These models naturally handle complex interactions,
and can provide insight into dominant patterns in the data.

Many of the loosely phrased explanations of patterns found in the Long
Creek data can be traced back, in part, to the insights generated by
...
Each of the tree models we prepared looking at dissolved oxygen are
based on predicting the probability of failing water quality standards,
not predicting absolute dissolved oxygen levels. This helps avoid
problems with data that is far from normally distributed, with rare
extreme low dissolved oxygen values.
...
#> 1  0.089504      0   1.00000 1.00058 0.021089
#> 2  0.084329      1   0.91050 0.90930 0.019596
#> 3  0.024114      2   0.82617 0.82870 0.019312
#> 4  0.022351      3   0.80205 0.81224 0.019497
#> 5  0.014269      5   0.75735 0.75791 0.018740
#> 6  0.013428      7   0.72881 0.74794 0.018762
#> 7  0.012959      9   0.70196 0.72884 0.018487
#> 8  0.010738     11   0.67604 0.69214 0.018282
#> 9  0.010369     12   0.66530 0.68003 0.018347
#> 10 0.010000     13   0.65493 0.67949 0.018352
```

These trees perform OK, with approximately 8.4 percent classification
error. The cross validation error does not flatten out, so there is
little evidence of significant overfitting here, but from an
interpretation perspective, the model is hard to summarize, so we do not
show a graphic of the classification tree.
...
## Side Bar: Frequency of Watershed Low Flow

Overall, about one sixth of summer days qualify as “low flow” based on
the flow index. We can determine that by looking at frequency of low
flow at the index station we used to develop the flow index, and
determining how often is below the cutoff cutoff value (0.096 meters, or
about 10cm) derived from the tree model.
...
So the cut point is at about 15% lowest flow conditions. We can graph
the relationship between flow index and flow at the upper watershed,
specifically Blanchette Brook, to understand how low-flow conditions
upstream may relate to low flow conditions in the middle watershed.
...
Low flow conditions in Blanchette Brook were common in some years, less
so in other years.
...
We might chose to develop a dissolved oxygen model that focuses on
*local* water depth rather than a stream-wide flow index. We develop
that model here to see
...
#> 
#>         CP nsplit rel error  xerror     xstd
#> 1 0.089504      0   1.00000 1.00022 0.021082
#> 2 0.084329      1   0.91050 0.90146 0.019190
#> 3 0.042665      2   0.82617 0.83021 0.019360
#> 4 0.029920      3   0.78350 0.79476 0.019973
#> 5 0.025702      4   0.75358 0.76549 0.018889
#> 6 0.011533      5   0.72788 0.74012 0.018686
#> 7 0.011377      6   0.71635 0.72551 0.018785
#> 8 0.010000      7   0.70497 0.72127 0.018813
```
...
#> 
#>         CP nsplit rel error  xerror     xstd
#> 1 0.117287      0   1.00000 1.00036 0.013809
#> 2 0.108776      1   0.88271 0.89041 0.013313
#> 3 0.045622      2   0.77394 0.77970 0.014343
#> 4 0.029404      3   0.72831 0.73503 0.014876
#> 5 0.016645      4   0.69891 0.70297 0.014149
#> 6 0.016496      5   0.68227 0.69039 0.014212
#> 7 0.011298      6   0.66577 0.67532 0.014068
#> 8 0.010000      7   0.65447 0.67164 0.014099
```
...
especially bad years, and then our most at-risk sites. So, the overall
story remains the same, except that temperature is no longer an
important explanatory variable. Percent saturation is driven by low flow
conditions, “bad” years, and at risk, small tributary, upstream sites.

But note that days with flow low enough to put percent saturation at
risk are somewhat more common with about one third of days falling below
the threshold used in this model.
...
#> 
#>         CP nsplit rel error  xerror      xstd
#> 1 0.191702      0   1.00000 1.00038 0.0063223
#> 2 0.077182      1   0.80830 0.80865 0.0095993
#> 3 0.021631      2   0.73112 0.73751 0.0107979
#> 4 0.021613      4   0.68785 0.70996 0.0105576
#> 5 0.019226      5   0.66624 0.68578 0.0104300
#> 6 0.014972      6   0.64701 0.66360 0.0100735
#> 7 0.010589      7   0.63204 0.64132 0.0101263
#> 8 0.010000      8   0.62145 0.62979 0.0102451
```
...
downstream tributaries (S01 and S03). 2016, 2017, and 2018 were bad
years, especially at our less vulnerable sites.
...
#> 
#>         CP nsplit rel error  xerror     xstd
#> 1 0.055124      0   1.00000 1.00029 0.058344
#> 2 0.025619      2   0.88975 0.89133 0.049154
#> 3 0.019038      5   0.81289 0.83687 0.049186
#> 4 0.016235      6   0.79386 0.83326 0.048039
#> 5 0.014514      7   0.77762 0.82782 0.047614
#> 6 0.014287      8   0.76311 0.81896 0.047202
#> 7 0.011952      9   0.74882 0.82682 0.047152
#> 8 0.010856     11   0.72492 0.81151 0.046178
#> 9 0.010000     12   0.71406 0.79788 0.045905
```
...
#> 
#>         CP nsplit rel error xerror    xstd
#> 1 0.026263      0   1.00000 1.0001 0.24199
#> 2 0.010000      2   0.94747 1.0363 0.24263
```
...
It’s not worth exploring trees for the chronic temperature limit, as
daily mean temperatures above that threshold almost never happen.
